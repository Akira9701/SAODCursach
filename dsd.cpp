...
void LRprint(vertex*);
void AVLSearch (vertex* p, int data);
...
int main()
{
 ...
 // Инициализация ncurses
    // nodelay(stdscr, TRUE);  // Устанавливаем неблокирующий режим
	...
	while (true) {
		...
		cout << "\t2. Sort the database using Heap Sort" << endl;
		cout << "\t3. Search within the sorted database (Binary Search - 3 tries max)" << endl;
		cout << "\t4. Search by criteria (partial match)" << endl;
		cout << "\t5. Exit" << endl;
		cout << "\tEsc. Exit" << endl;
		cout << "\t----------------------------------------------------------" << endl;
		    ...
		    do {
        enter = getch();  // Здесь можно считывать символы без блокирования
		...
		case 51:
			...
			if(strcmp(depositor,"0")!=0){
				...
				char buffer[32];
				if(search == -1){
					cout << "Òàêîãî âêëàä÷èêà íå ñóùåñòâóåò" << endl;
				}
				else{	
...
void addAVL(list *data, vertex *&point){
	if (point == NULL) {
		...
		if(point->data->deposit == data->data->deposit){
...
void Print_base(list *base) {
    ...
    cbreak();   // Включить режим cbreak (без буферизации)
    keypad(stdscr, TRUE);  // Включить обработку специальных клавиш

    printw("1 for 4000 / 2 for 20 per click / 3 for nothing\n");
    refresh();
    n = getch(); // Используем getch() вместо cin

    if (n == '1') {
        i = 0;
        while (i < N) {
            printw("%d  %s  %s  %s  %s\n", i, base->data->depositor, base->data->deposit, base->data->date, base->data->lawyer);
            refresh();
            i++;
            
            // Обработка нажатия клавиши 'e' для выхода
            int result = getch();
            if (result != ERR) {
                printw("\nKey '%c' was pressed. Press 'e' key to exit.\n", result);
                refresh();
                char something = getch();
                if (something == 'e') {
        ...
        for (int i = 0; i < N; i = i + 20) {
            while ((g++) < 20) {
                printw("%d  %s  %s  %s  %s\n", i + g, base->data->depositor, base->data->deposit, base->data->date, base->data->lawyer);
                refresh();
                base = base->next;
            }
            if (g >= 20) {
                printw("y for continue\n");
                refresh();
                n = getch();
                if (n != 'y') {
                    endwin();  // Завершение работы с ncurses
...
void Print_sort_index_mas(list **mas) {
    ...
    if (n == '1') {
        i = 0;
        while (i < N) {
            printw("%d  %s  %s  %s  %s\n", i, mas[i]->data->depositor, mas[i]->data->deposit, mas[i]->data->date, mas[i]->data->lawyer);
            refresh();
            i++;
        ...
        for (int i = 0; i < N; i = i + 20) {
            while ((g++) < 20) {
                printw("%d  %s  %s  %s  %s\n", i + g, mas[i + g]->data->depositor, mas[i + g]->data->deposit, mas[i + g]->data->date, mas[i + g]->data->lawyer);
                refresh();
            }
            if (g >= 20) {
                printw("y for continue\n");
